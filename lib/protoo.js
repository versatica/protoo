...
var debugerror = require('debug')('protoo:ERROR');
...
protoo.addMethod = function(method) {
	if (Application.prototype[method]) {
		debugerror('addMethod() | method "%s" already exists in Application prototype', method);
		return;
	}

	if (Router.prototype[method]) {
		debugerror('addMethod() | method "%s" already exists in Router prototype', method);
		return;
	}

	if (Route.prototype[method]) {
		debugerror('addMethod() | method "%s" already exists in Route prototype', method);
		return;
	}

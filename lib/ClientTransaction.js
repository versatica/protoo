...
var debug = require('debug')('protoo:ClientTransaction'),
	debugerror = require('debug')('protoo:ERROR:ClientTransaction'),
 ...
 */
	...
	SESSION_SENDING_TIMEOUT = 120000,  // 2 minutes.
	ERRORS = {
		TIMEOUT:     1,
		CANCELED:    2,
		OFFLINE:     3,
...
function ClientTransaction(peer, req, onended) {
	...
	this.timer = setTimeout(function () {
		...
		if (self.isSession && self.sent) {
			// Send 'end' request.
			self.peer.send(Request.factory({
				method: 'end',
				data: {
					sessionPath: self.req.data.sessionPath
				}
			}));
		}
	...
	// Peer is online.
	if (peer.online) {
		debug('peer is online, sending the request now');
		send.call(this);
...
ClientTransaction.prototype.handleResponse = function (res) {
	/**
	 * Validate status.
	 */

	if (this.ended) {
		debug('handleResponse() | transaction ended, ignoring received response: %s', res);
		return;
	}
...
ClientTransaction.prototype.cancel = function (endRequest) {
	if (this.ended) {
		return;
	}
	...
	if (this.isSession && this.sent) {
		endRequest = endRequest || Request.factory({
			method: 'end'
		});

		// Ensure .data.sessionPath is set.
		endRequest.data.sessionPath = self.req.data.sessionPath;

		self.peer.send(endRequest);
	}
};

 ...
 * 'error'
 *
 * Fired upon an usage or internal error. If the user does not install a
 * listener for this event then the error is thrown.
 *
 * @event error
 * @param {Error} error.
 */
...
function Application() {
	...
	this.peerManager.on('error', function(error) {
		debugerror('PeerManager emits "error" event: %o', error);
...
Application.prototype.websocket = function(httpServer, connectionListener) {
	...
	wsServer.on('error', function(error) {
		debugerror('WebSocketServer emits "error" event: %o', error);
		handleError.call(this, error);
	}.bind(this));
 ...
 *
 * @method online
 * @param {Function} onlineListener
 * @param {Peer} peer  The online Peer instance.
 * @chainable
 */
Application.prototype.online = function(onlineListener) {
	debug('online()');

	if (typeof onlineListener !== 'function') {
		throw new Error('onlineListener must be a function');
	}

	this.peerManager.setOnlineListener(onlineListener);
	return this;
};


/**
 * Peer offline handler.
 *
 * @method offline
 * @param {Function} offlineListener
 * @param {Peer} peer  The offline Peer instance.
 * @chainable
 */
Application.prototype.offline = function(offlineListener) {
	debug('offline()');

	if (typeof offlineListener !== 'function') {
		throw new Error('offlineListener must be a function');
	}

	this.peerManager.setOfflineListener(offlineListener);
	return this;
};

...
function Application() {
	...
	this.peerManager = new PeerManager({
		online: function(peer) {
			emit.call(self, 'online',  peer);
		},

		offline: function(peer) {
			emit.call(self, 'offline', peer);
		},
 ...
 * @chainable
 */
Application.prototype.websocket = function app_websocket(httpServer, requestListener) {
	debug('websocket()');
...
function handleRequest(peer, req, done) {
	...
	// Attach the peer to the request.
	req.peer = peer;
	...
	function onerror(error) {
		...
		emit.call(self, 'error:route', error);
	}
...
function emit(event) {
	...
	try {
		...
		logError.call(this, error, 'error thrown during "' + event + '" event');
	}
}

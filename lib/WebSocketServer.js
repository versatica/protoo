...
WebSocketServer.prototype.onAuthorize = function(info, cb) {
	...
	debug('onAuthorize() emitting "ws:connecting" on app');
	try {
		this.app.emit('ws:connecting', connectingInfo, acceptCb, rejectCb, waitCb);
	}
	catch(error) {
		debug('onAuthorize() error catched during "ws:connecting" event on app: %s', error);
	...
	if (! done && ! wait_called) {
		debug('onAuthorize() error: no callback called during "ws:connecting" event on app');
		this.app.emit('error', Error('no callback called during "ws:connecting" event on app'));
...
WebSocketServer.prototype.onConnection = function(websocket) {
	...
	// Create a new Protoo WebSocket connection.

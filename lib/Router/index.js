...
Router.prototype.param = function router_param(name, fn) {
...
Router.prototype.handle = function router_handle(req, done) {
	...
	function next(err) {
		...
		while (match !== true && idx < stack.length) {
			debug('handle().next() | in stack [idx:%d]', idx);
	...
	function trim_prefix(layer, layerError, layerPath, path) {
		...
		if (c && c !== '/') {
			debugerror('handle().trim_prefix() | path does not begin with "/"');
		...
		if (layerPath.length !== 0) {
			debug('handle().trim_prefix() | trim prefix "%s" from path "%s"', layerPath, req.path);
			removed = layerPath;
		...
		debug('handle().trim_prefix() | [name:%s, layerPath:%s, reqPath:%s, reqOriginalPath:%s]', layer.name, layerPath, req.path, req.originalPath);

		if (layerError) {
			debug('handle().trim_prefix() | layer error');
...
Router.prototype.use = function router_use(fn) {
	var offset = 0,
		path = '/';
...
Router.prototype.route = function router_route(path) {

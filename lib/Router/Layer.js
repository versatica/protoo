...
Layer.prototype.handle_request = function(req, next) {
	var fn = this.handle;

	if (fn.length > 2) {
		debugerror('handle_request() | non a standard request handler, next');
		// Not a standard request handler.
		return next();
	}

	try {
		debug('handle_request() | calling fn: %o', fn);
		fn(req, next);
		debug('handle_request() | called fn: %o', fn);
	} catch(error) {
		debugerror('handle_request() | calling fn throws error: %o', error);
		next(error);
	}
};
